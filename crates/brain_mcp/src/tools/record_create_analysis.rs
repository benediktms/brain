//! `records.create_analysis` MCP tool — thin wrapper over
//! `DaemonClient::analyses_create`.
//!
//! Accepts `title` plus either `text` (plain UTF-8) or `data` (base64).
//! The body is decoded to bytes here and passed through `RecordsCreateParams`
//! to the daemon, which writes to the object store and emits the event.
...
impl McpTool for RecordCreateAnalysis {
    ...
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            name: self.name().into(),
            description: "Create a new analysis record. Writes content to the object store with kind='analysis'.".into(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "title": {
    ...
    fn call<'a>(
        ...
        Box::pin(async move {
            ...
            let (record, content_hash, size) =
                match ctx.with_client(|c| c.analyses_create(wire_params)).await {
                    Ok(r) => r,
                    Err(err) => {
