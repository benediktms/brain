//! `records.create_document` MCP tool — thin wrapper over
//! `DaemonClient::documents_create`.
//!
...
impl McpTool for RecordCreateDocument {
    ...
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            name: self.name().into(),
            description: "Create a new document record. Writes content to the object store with kind='document'.".into(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "title": {
    ...
    fn call<'a>(
        ...
        Box::pin(async move {
            ...
            let (record, content_hash, size) =
                match ctx.with_client(|c| c.documents_create(wire_params)).await {
                    Ok(r) => r,
                    Err(err) => {
