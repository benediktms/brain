...
fn create_remote(ctx: &DocumentCtx, params: CreateParams) -> Result<()> {
    ...
    let (summary, _content_hash, _size) = client
        .documents_create(wire_params)
        .map_err(|e| anyhow::anyhow!("DocumentsCreate rpc failed: {e}"))?;

    if ctx.json {
        println!("{}", serde_json::to_string_pretty(&summary)?);
    } else {
        println!("Created document {}", summary.record_id);
...
pub fn create(ctx: &DocumentCtx, params: CreateParams) -> Result<()> {
    if params.remote {
    ...
    let record = stores
        .records
        .create_document(create_params, &stores.objects)
    ...
    if ctx.json {
        ...
        println!("Created document {record_id}");
        if target_stores.is_some() {
