...
fn create_remote(ctx: &PlanCtx, params: CreateParams) -> Result<()> {
    ...
    let (summary, _content_hash, _size) = client
        .plans_create(wire_params)
        .map_err(|e| anyhow::anyhow!("PlansCreate rpc failed: {e}"))?;

    if ctx.json {
        println!("{}", serde_json::to_string_pretty(&summary)?);
    } else {
        println!("Created plan {}", summary.record_id);
