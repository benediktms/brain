...
impl TaskStore {
    ...
    /// must use the inherent delegation methods on `TaskStore` (or call
    ...
    /// Construct a `TaskStore` scoped to a different brain on the same
    ...
    pub fn resolve_task_id(&self, input: &str) -> Result<String> {
        ...
        self.db
            .with_read_conn(move |conn| {
                ...
                // Propagate the resolver's errors (BrainRegistry ambiguity, DB errors) instead
                // of swallowing them with `unwrap_or` — the explicit error from the persistence
                // layer is the whole point of the defense-in-depth check at that boundary.
                let effective_brain_id = if !brain_id.is_empty() {
                    queries::resolve_brain_from_prefix(conn, input)?.or(Some(brain_id))
                } else {
    ...
    /// Transfer a task to a different brain in a single atomic transaction.
    ///
    /// Preserves the `task_id`. Updates `tasks.brain_id` and `tasks.display_id`,
    /// SQLite `chunks.brain_id`, `files.brain_id`, and `records.brain_id`. Inserts
