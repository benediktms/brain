...
pub struct TaskCreatedPayload {
    ...
    /// Stable hash-based short display ID (hex only, e.g. "a3f"). Prefix applied at runtime.
...
pub fn now_ts() -> i64 {
    brain_core::utils::now_ts()
}

// ---------------------------------------------------------------------------
// File I/O — stays in brain_tasks (uses local error + std::io)
// ---------------------------------------------------------------------------
