...
pub(super) struct RecordCreateDocument;

impl RecordCreateDocument {
    fn execute(&self, params: Value, ctx: &McpContext) -> ToolCallResult {
        ...
        let record = match records.create_document(create_params, &ctx.stores.objects) {
            Ok(r) => r,
        ...
        let policy = RecordKind::from("document").policy();
                ...
                .upsert_record_chunk(&record_file_id, &abstract_text)
        {
            tracing::warn!(
                record_id = %record_id,
                error = %e,
                "record_create_document: failed to write L0 abstract to FTS"
            );
        }
        if policy.embed {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_create_document_writes_fts_and_lod() {
        ...
        let result = registry
            .dispatch(
                ...
                json!({
                    "title": "Doc Title",
                    "text": "Document body for FTS and LOD"
                }),
                &ctx,
            )
            .await;
        ...
        assert!(lod.is_some(), "document record should write L0 LOD chunk");
    }
}

impl McpTool for RecordCreateDocument {
    ...
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            name: self.name().into(),
            description:
                "Create a new document record. Writes content to the object store with kind='document'."
                    .into(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    ...
                    "data": {
                        "type": "string",
                        "description": "Base64-encoded document bytes. Provide either 'data' or 'text', not both."
                    },
                    "text": {
                        "type": "string",
                    ...
                    "task_id": {
                        "type": "string",
                        "description": "Optional task ID this document is associated with"
                    },
                    "tags": {
                        "type": "array",
                        "items": { "type": "string" },
