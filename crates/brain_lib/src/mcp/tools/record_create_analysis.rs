...
impl RecordCreateAnalysis {
    fn execute(&self, params: Value, ctx: &McpContext) -> ToolCallResult {
        ...
        let record = match records.create_analysis(create_params, &ctx.stores.objects) {
            Ok(r) => r,
        ...
        let policy = RecordKind::from("analysis").policy();
...
mod tests {
    ...
    #[tokio::test]
    async fn test_create_analysis_writes_fts_and_lod() {
        ...
        let result = registry
            .dispatch(
                ...
                json!({
                    ...
                    "text": "Analysis body for FTS and LOD"
                }),
                &ctx,
            )
            .await;
        ...
        assert_eq!(chunk_count, 1, "analysis record should write one FTS chunk");
        ...
        assert!(lod.is_some(), "analysis record should write L0 LOD chunk");
    }
}
